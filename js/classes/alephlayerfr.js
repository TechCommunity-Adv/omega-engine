class AlephLayerFrance
{
    constructor()
    {
        this.aleph = new Decimal(0);
        this.upgrades = {
            alephGain: new AlephUpgradeFrance("Increase your aleph gain", level => Decimal.pow(1.215, level).mul(100),
                level => Decimal.pow(1.2 + game.restackLayer.permUpgrades.aleph.apply(), level)),
            alephGainBonus: new AlephUpgradeFrance("Get a Bonus to aleph gain",
            ...
            alephBoost: new AlephUpgradeFrance("Gain more aleph based on the log(ℵ) you have",
                level => new Decimal(1e6).pow(Decimal.pow(1.5, level)),
                level => new Decimal(1).add(Decimal.max(0, game.alephLayerFr.aleph).add(1).log10().mul(level).mul(0.05)).pow(2.5)),
            deltaBoost: new AlephUpgradeFrance("Gain more &delta;",
                level => Decimal.pow(1e5, level).mul(1e3),
                level => Decimal.pow(10, level), {
                    maxLevel: 4
                }),
            powerGenerators: new AlephUpgradeFrance("All Power Generators on every Layer are stronger",
                level => Utils.createValueDilation(Decimal.pow(1e5, Decimal.pow(level, 1.5)).mul(1e20), 0.001),
                level => Decimal.pow(1.5, level)),
            prestigeNoPowerBoost: new AlephUpgradeFrance("Increase Prestige Reward on all Layers that don't have Power Generators",
                level => Decimal.pow(1e8, level).mul(1e22),
                level => Decimal.pow(2, level), {
                    maxLevel: 3
                }),
            alephBoost2: new AlephUpgradeFrance("Gain more aleph based on the log(log(&alpha;)) you have",
            ...
            betterBetaFormula: new AlephUpgradeFrance("The &beta; Prestige Formula is better",
                level => new Decimal(1e90),
                level => new Decimal(1).add(level.mul(0.12)), {
                    maxLevel: 1,
                    getEffectDisplay: effectDisplayTemplates.numberStandard(2, "^")
                }),
            prestigeRewards: new AlephUpgradeFrance("Increase the Prestige Reward of all Layers",
                level => Utils.createValueDilation(Decimal.pow(1e30, level).mul(1e100), 0.005, new Decimal("1e650")),
                level => Decimal.pow(1.6, level)),
            layerExponentialBoost: new AlephUpgradeFrance("Increase the exponential difference of boosts between layers, resulting in a large boost!",
